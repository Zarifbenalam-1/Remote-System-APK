...
class AudioCommand(private val context: Context) : CommandHandler {
    ...
    override fun execute(params: JSONObject, clientSocketId: String, socket: Socket) {
        ...
        when (action) {
            "start" -> startAudioStream(params, clientSocketId, socket)
            "stop" -> stopAudioStream(clientSocketId, socket)
            "record" -> recordAudio(params, clientSocketId, socket)
            "get_info" -> getAudioInfo(clientSocketId, socket)
            else -> sendErrorResponse("Unknown action: $action", clientSocketId, socket)
        }
    }

    private fun startAudioStream(params: JSONObject, clientSocketId: String, socket: Socket) {
        if (!checkAudioPermission()) {
            sendErrorResponse("Audio recording permission not granted", clientSocketId, socket)
            return
        }

        if (isRecording.get()) {
            sendErrorResponse("Audio stream already active", clientSocketId, socket)
            return
        }
        ...
        try {
            ...
            startAudioRecording(duration, quality, clientSocketId, socket)

        } catch (e: Exception) {
            Log.e(tag, "Error starting audio stream", e)
            sendErrorResponse("Failed to start audio stream: ${e.message}", clientSocketId, socket)
        }
    }

    private fun stopAudioStream(clientSocketId: String, socket: Socket) {
        try {
            stopRecording()
            ...
            sendErrorResponse("Failed to stop audio stream: ${e.message}", clientSocketId, socket)
        }
    }

    private fun recordAudio(params: JSONObject, clientSocketId: String, socket: Socket) {
        if (!checkAudioPermission()) {
            sendErrorResponse("Audio recording permission not granted", clientSocketId, socket)
            return
        }
        ...
        if (isRecording.get()) {
            sendErrorResponse("Audio recording already in progress", clientSocketId, socket)
            return
        }
        ...
        try {
            ...
            recordAudioClip(duration, quality, clientSocketId, socket)

        } catch (e: Exception) {
            Log.e(tag, "Error recording audio", e)
            sendErrorResponse("Failed to record audio: ${e.message}", clientSocketId, socket)
        }
    }

    private fun getAudioInfo(clientSocketId: String, socket: Socket) {
        try {
            val response = JSONObject()
            val data = JSONObject()
            ...
            sendErrorResponse("Failed to get audio info: ${e.message}", clientSocketId, socket)
        }
    }

    private fun startAudioRecording(duration: Int, quality: String, clientSocketId: String, socket: Socket) {
        try {
            val bufferSize = AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)
            
            if (bufferSize == AudioRecord.ERROR || bufferSize == AudioRecord.ERROR_BAD_VALUE) {
                sendErrorResponse("Failed to get audio buffer size", clientSocketId, socket)
                return
            }
            ...
            if (audioRecord?.state != AudioRecord.STATE_INITIALIZED) {
                sendErrorResponse("Failed to initialize audio recorder", clientSocketId, socket)
                return
            }
            ...
            recordingThread = Thread {
                streamAudioData(bufferSize, quality, duration, clientSocketId, socket)
            }
            recordingThread?.start()

        } catch (e: SecurityException) {
            sendErrorResponse("Audio permission denied", clientSocketId, socket)
        } catch (e: Exception) {
            Log.e(tag, "Error starting audio recording", e)
            sendErrorResponse("Failed to start audio recording: ${e.message}", clientSocketId, socket)
        }
    }

    private fun recordAudioClip(duration: Int, quality: String, clientSocketId: String, socket: Socket) {
        try {
            val bufferSize = AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)
            audioRecord = AudioRecord(audioSource, sampleRate, channelConfig, audioFormat, bufferSize * 2)
            ...
            recordingThread = Thread {
                recordAudioClipData(bufferSize, duration, quality, clientSocketId, socket)
            }
            recordingThread?.start()
            ...
            sendErrorResponse("Failed to record audio clip: ${e.message}", clientSocketId, socket)
        }
    }

    private fun streamAudioData(bufferSize: Int, quality: String, duration: Int, clientSocketId: String, socket: Socket) {
        ...
        while (isRecording.get()) {
            ...
            if (bytesRead > 0) {
                ...
                if (frameCount * bufferSize * 1000 / (sampleRate * 2) >= frameInterval) {
                    sendAudioFrame(audioBuffer, bytesRead, clientSocketId, socket, true)
                    frameCount = 0
                }
            }
    ...
    private fun recordAudioClipData(bufferSize: Int, duration: Int, quality: String, clientSocketId: String, socket: Socket) {
        val audioBuffer = ByteArray(bufferSize)
        ...
        if (audioBytes.isNotEmpty()) {
            sendAudioFrame(audioBytes, audioBytes.size, clientSocketId, socket, false)
        }
    }

    private fun sendAudioFrame(audioData: ByteArray, size: Int, clientSocketId: String, socket: Socket, isStream: Boolean) {
        try {
